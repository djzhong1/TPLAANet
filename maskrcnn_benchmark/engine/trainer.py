...
def do_train(
    ...
    meters = MetricLogger(delimiter="  ", summary_writer=tb_logger)
    max_iter = len(data_loader)
    start_iter = arguments["iteration"]
    model.train()
    start_training_time = time.time()
    end = time.time()
    # train_iter = data_loader.__iter__()

    # for iteration in range(max_iter):
    #     try:
    #         images, targets, _ = next(train_iter)
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if local_rank == 0 and (iteration % cfg.SOLVER.DISPLAY_FREQ == 0 or iteration == (max_iter - 1)):
            ...
            #     meters.summary(tag, value.item(), iteration)
        
        if iteration % 20 == 0:
            meters.summary(iteration) 
