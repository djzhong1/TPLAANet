...
def mask2polygon(mask, box, im_size, threshold=0.5, output_folder=None):
    ...
    #cv2.write('mask.jpg', poly_map*255)
    if "total_text" in output_folder or "cute80" in output_folder or "ctw" in output_folder:
        ...
        corners = cv2.boxPoints(rect).reshape((-1,2))
        #corners = [x * 1.0 for x in pts].reshape((-1,2))
        corners[:,0] = corners[:,0] + box[0]
        corners[:,1] = corners[:,1] + box[1]
        polygon = list(corners.reshape((-1,)))
        polygon = list(map(int, polygon))
        # find minimum area bounding box.
        # rect = cv2.minAreaRect(hull)
        # corners = cv2.boxPoints(rect)
...
def format_output(out_dir, boxes, img_name):
    with open(
        ...
        for i, box in enumerate(boxes):
            ...
            save_dict = {}
            if "total_text" in out_dir or "cute80" in out_dir or "ctw" in out_dir:
...
def visualization(image_path, image, polygons, resize_ratio, colors, char_polygons=None, words=None):
    draw = ImageDraw.Draw(image, "RGB")
    #font = ImageFont.truetype(ImageFont., 20)
    font = ImageFont.truetype("../maskrcnn_benchmark/fonts/ARLRDBD.TTF", 15)
    for idx, polygon in enumerate(polygons):
        ...
        # print(polygon)
        color = '#33FF33'
        draw.text((polygon[0],polygon[1]-23), words[idx], font=font, fill=(0, 0, 255))
...
def vis_seg_map(image_path, seg_map, rotated_boxes, polygons_this_image, proposals, vis_dir):
    ...
    if "total_text" in vis_dir or "cute80" in vis_dir or "ctw" in vis_dir:
    ...
    if (not cfg.MODEL.TRAIN_DETECTION_ONLY):
        for image_path, prediction in predictions.items():
            ...
            for k, box in enumerate(boxes):
                ...
                # x_min = polygon[:, 0].min()
                # x_max = polygon[:, 0].max()
                tem_polygon = np.array(polygon)
                x_min = tem_polygon[0::2].min()
                x_max = tem_polygon[0::2].max()
                y_min = tem_polygon[1::2].min()
                y_max = tem_polygon[1::2].max()
            ...
            if vis:
                ...
                if False:
                    visualization(image_path, img, polygons, resize_ratio, colors, char_polygons, seq_words)
                else:
                    visualization(image_path, img, polygons, resize_ratio, colors, words=seq_words)
