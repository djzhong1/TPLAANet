...
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #     self.resolution_h = cfg.MODEL.ROI_MASK_HEAD.POOLER_RESOLUTION_H
        #     self.resolution_w = cfg.MODEL.ROI_MASK_HEAD.POOLER_RESOLUTION_W
        # else:
        #     self.resolution_h = cfg.MODEL.ROI_MASK_HEAD.POOLER_RESOLUTION
        #     self.resolution_w = cfg.MODEL.ROI_MASK_HEAD.POOLER_RESOLUTION
        pooler = Pooler(
        ...
        if self.cfg.MODEL.ROI_BOX_HEAD.MIX_OPTION == 'CAT':
            input_size = (cfg.MODEL.BACKBONE.OUT_CHANNELS + 1) * self.resolution ** 2
        else:
            input_size = cfg.MODEL.BACKBONE.OUT_CHANNELS * self.resolution ** 2
        representation_size = cfg.MODEL.ROI_BOX_HEAD.MLP_HEAD_DIM
        self.mix_option = cfg.MODEL.ROI_BOX_HEAD.MIX_OPTION
        self.pooler = pooler
        # self.fc6_new = nn.Linear(input_size, representation_size)
        # self.fc7_new = nn.Linear(representation_size, representation_size)
        self.fc6_new = nn.Linear(input_size, representation_size)
        ...
        #for l in [self.fc6_new, self.fc7_new]:
        for l in [self.fc6_new, self.fc7]:
    ...
    def feature_mask(self, x, proposals):
        ...
        for proposal in proposals:
            segmentation_masks = proposal.get_field("shrink_masks")
            boxes = proposal.bbox.to(torch.device("cpu")).int()
            for segmentation_mask, box in zip(segmentation_masks, boxes):
                #cropped_mask = segmentation_mask.crop(box)
                #scaled_mask = cropped_mask.resize((self.resolution_w*3, self.resolution_h*3))
                mask = segmentation_mask.convert(mode="mask")
                # a = np.concatenate((mask[:,:,np.newaxis],mask[:,:,np.newaxis],mask[:,:,np.newaxis]), 2)
                # cv2.imwrite('./maski1.jpg', a*255)
                mask = mask[box[1]:box[3], box[0]:box[2]] #ymin:ymax, xmin:max
                # a = np.concatenate((mask[:,:,np.newaxis],mask[:,:,np.newaxis],mask[:,:,np.newaxis]), 2)
                # cv2.imwrite('./maski2.jpg', a*255)

                mask = F.interpolate(mask.float()[None,None,:,:], (self.resolution, self.resolution)).squeeze(0).squeeze(0)
                #cv2.imwrite('./maski3.jpg', a*255)
                masks.append(mask)

        x = torch.cat([x, torch.stack(masks).unsqueeze(1).to(x.device)], dim=1)


        return x


    def feature_mask_origin(self, x, proposals):
        masks = []
        for proposal in proposals:
