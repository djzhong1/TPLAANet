...
def unclip(box, expand_ratio=4):
    poly = ShapePolygon(box)
...
class SegmentationMask(object):
    ...
    def get_shrink_polygons(self, shrink_ratio, size, ignore_difficult=False):
        # size: w h
        # assert self.size[0] == seg_size[1]
        # assert self.size[1] == seg_size[0]
        # height, width = seg_size[0], seg_size[1]
        ...
        inds = []
        width, height = size
        # origin_map = np.zeros((height, width), dtype=np.uint8)
        # seg_map = np.zeros((height, width), dtype=np.uint8)

        for ind, poly in enumerate(self.polygons):
            poly = poly.get_polygons()[0]
            poly = poly.reshape((-1, 2)).numpy()
            
            #cv2.fillPoly(origin_map, [poly.astype(int)], 1)
            ...
            polygons.append([out])
            inds.append(ind)
            #cv2.fillPoly(seg_map, [out.astype(np.int32)], 1)

            # seg_map = np.zeros((height, width), dtype=np.uint8)
            # cv2.fillPoly(seg_map, [out.astype(np.int32)], 1)
            # #seg_map = seg_map[y_min:y_max, x_min:x_max]
            # cv2.imwrite('./shrinki.jpg', seg_map*255)

        # a=np.concatenate((origin_map[:,:,np.newaxis],origin_map[:,:,np.newaxis],origin_map[:,:,np.newaxis]), 2)
        # cv2.imwrite('./origin.jpg', a*255)

        # a=np.concatenate((seg_map[:,:,np.newaxis],seg_map[:,:,np.newaxis],seg_map[:,:,np.newaxis]), 2)
        # cv2.imwrite('./shrink.jpg', a*255)
        return polygons, np.array(inds)
...
class Polygons(object):
    ...
    def convert(self, mode):
        ...
        if mode == "mask":
            ...
            poly = self.polygons[0].numpy().astype(np.int32).reshape(-1, 2)
            cv2.fillPoly(mask, [poly], 1)
            # a = np.concatenate((mask[:,:,np.newaxis],mask[:,:,np.newaxis],mask[:,:,np.newaxis]), 2)
            # cv2.imwrite('./maski.jpg', a*255)
            # try:
            #     rles = mask_utils.frPyObjects(
            #         [p.numpy() for p in self.polygons[0]], height, width
            #     )
            # except:
            #     print([p.numpy() for p in self.polygons])
